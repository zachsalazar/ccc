BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["CCC_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["CCC_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["CCC_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["CCC_NOLINEWRAP"] != "" && ENVIRON["CCC_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
